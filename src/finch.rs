...
impl PreclusterDistanceFinder for FinchPreclusterer {
    fn distances(&self, genome_fasta_paths: &[&str]) -> SortedPairGenomeDistanceCache {
        distances(genome_fasta_paths, self.min_ani, self.num_kmers, self.kmer_length)
    }
}

pub fn distances(genome_fasta_paths: &[&str], min_ani: f32, num_kmers: usize, kmer_length: u8)
-> SortedPairGenomeDistanceCache {
    ...
    let sketches_result = finch::sketch_files(
        genome_fasta_paths,
        &sketch_params, 
        &filters);
    info!("Finished sketching genomes");

    let sketches = sketches_result
        .expect("Failed to sketch genomes with finch");
    ...
    for (i, sketch1) in sketches.iter().enumerate() {
        for (j, sketch2) in sketches[(i+1)..sketches.len()].iter().enumerate() {
            ...
            let distance = 1.0-finch::distance::distance(sketch1,sketch2,false)
                .expect(&format!("Failed to compare finch sketches {} and {}", i, genome_index2))
                .mash_distance;
            debug!("Comparing {} and {}, distance {}", genome_fasta_paths[i], genome_fasta_paths[genome_index2], distance);
...
mod tests {
    ...
    fn test_hello_world() {
        ...
        let distances1 = distances(&["tests/data/set1/1mbp.fna","tests/data/set1/500kb.fna"],0.9,1000,21);
        let mut expected1 = SortedPairGenomeDistanceCache::new();
        expected1.insert((0,1),Some(0.9808188));
        assert_eq!(expected1,distances1);

        let distances2 = distances(&["tests/data/set1/1mbp.fna","tests/data/set1/500kb.fna"],0.99,1000,21);
