...
pub fn read_genome_info_file(file_path: &str) -> Result<checkm::CheckMResult, String> {
    ...
    let mut parse_result = rdr
        .expect(&format!("Failed to parse genomeInfo file {}", file_path));
    if parse_result.headers().expect("Failed to find headers in genomeInfo file") != vec!["genome","completeness","contamination"] {
    ...
    for result in parse_result.records() {
        ...
        if res.len() != 3 {
            return Err(format!("Parsing error in genomeInfo file - didn't find 3 columns in line {:?}", res).to_string());
        }
        let completeness: f32 = res[1].parse::<f32>().expect("Error parsing completeness in checkm tab table");
        let contamination: f32 = res[2].parse::<f32>().expect("Error parsing contamination in checkm tab table");
        ...
        match qualities.insert(
            ...
            None => {},
            Some(_) => {
                return Err(format!("The genome {} was found multiple times in the checkm file {}", res[0].to_string(), file_path).to_string());
            }
        };
        total_seen += 1;
    }
